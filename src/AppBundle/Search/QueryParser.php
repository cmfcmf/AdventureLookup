...
class QueryParser
{
    ...
    public static function parse(string $query, $fields): array
    {
        ...
        $tokens = self::convertBracketsToArrays($tokens);
    ...
    public static function balanceBrackets(array $tokens): array
    {
        $nOpen = 0;
        $nClose = 0;
        $sum = 0;
        ...
        if ($minSum < 0) {
            ...
            // adjust $sum based on number of newly inserted brackets.
            $sum -= $minSum;
        }
        if ($sum > 0) {
    ...
    public static function convertBracketsToArrays(array $tokens): array
    {
        return self::_convertBracketsToArrays((new ArrayObject($tokens))->getIterator());
    }

    private static function _convertBracketsToArrays(\ArrayIterator $tokens): array
    {
        $tree = [];
        ...
        while ($tokens->valid()) {
            ...
            if ($token instanceof SpecialToken) {
                if ('(' === $token->content) {
                    $token = self::_convertBracketsToArrays($tokens);
                    if (1 === count($token)) {
                        // unwrap arrays with just one element (i.e., a single value enclosed by brackets '( A )')
                        $token = $token[0];
                    }
                } elseif (')' === $token->content) {
                    return $tree;
